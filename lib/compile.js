(function(root, factory) {
    ...
    function Compile(structure, opts) {
        ...
        function buildToken(ruleName, token, env) {
            ...
            if (token.literal) {
                ...
                if (token.literal.length === 1 || result.config.lexer) {
                    return token.insensitive ? insensitiveLiteral(token.literal) : token;
                }
        ...
        function charLiteral(s) {
            return {
                literal: s
            };
        }

        function insensitiveLiteral(s) {
        ...
        function buildStringToken(ruleName, token, env) {
            ...
            produceRules(newname, [
                {
                    tokens: token.literal.split("").map(token.insensitive ? insensitiveLiteral : charLiteral),
